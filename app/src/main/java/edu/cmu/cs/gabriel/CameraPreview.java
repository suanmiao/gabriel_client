  ...
  public void surfaceCreated(SurfaceHolder holder) {
    ...
    if (mCamera != null) {
      try {
        ...
        if (this.supportingFPS == null) this.supportingFPS = supportedFps;
        ...
        if (this.supportingSize == null) this.supportingSize = supportedSizes;
        index = 0;
        int sizeDiff = Integer.MAX_VALUE;
        for (int i = 0; i < supportedSizes.size(); i++) {
          Camera.Size size = supportedSizes.get(i);
          int diff =
              Math.abs(size.width - Const.IMAGE_WIDTH) + Math.abs(size.height - Const.IMAGE_HEIGHT);
          if (diff < sizeDiff) {
            sizeDiff = diff;
            index = i;
          }
